...
public class MakeAstronomyCategoryProvider
{
    public BookCategoryModel makeAstronomyCategory(BookLangHelper helper) {
        helper.category("astronomy_category"); //tell our lang helper the category we are in
        ...
        return BookCategoryModel.create(new ResourceLocation(SensMagicMod.MODID, helper.category), helper.categoryName())
                .withIcon(ModBlockss.WIZARD_ASTRONOMY_STATION.getId().toString()); //the icon for the category. In this case we simply use an existing item.
    }
}
