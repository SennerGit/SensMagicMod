...
public class MakeFabricCategoryProvider
{
    public BookCategoryModel makeFabricCategory(BookLangHelper helper) {
        helper.category("fabric_category"); //tell our lang helper the category we are in
        ...
        return BookCategoryModel.create(new ResourceLocation(SensMagicMod.MODID, helper.category), helper.categoryName())
                .withIcon(ModBlockss.WIZARD_FABRIC_STATION.getId().toString()); //the icon for the category. In this case we simply use an existing item.
    }
}
