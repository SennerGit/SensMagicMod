...
public class MakeStoneCuttingCategoryProvider
{
    public BookCategoryModel makeStoneCuttingCategory(BookLangHelper helper) {
        helper.category("stone_cutter_category"); //tell our lang helper the category we are in
        ...
        return BookCategoryModel.create(new ResourceLocation(SensMagicMod.MODID, helper.category), helper.categoryName())
                .withIcon(ModBlockss.WIZARD_STONE_CUTTER.getId().toString()); //the icon for the category. In this case we simply use an existing item.
    }
}
