...
public class MakeAgricultureCategoryProvider
{
    public BookCategoryModel makeAgricultureCategory(BookLangHelper helper) {
        helper.category("agriculture_category"); //tell our lang helper the category we are in
        ...
        return BookCategoryModel.create(new ResourceLocation(SensMagicMod.MODID, helper.category), helper.categoryName())
                .withIcon(ModBlockss.WIZARD_AGRICULTURE_STATION.getId().toString()); //the icon for the category. In this case we simply use an existing item.
    }
}
