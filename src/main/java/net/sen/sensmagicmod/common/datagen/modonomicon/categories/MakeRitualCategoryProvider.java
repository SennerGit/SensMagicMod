...
public class MakeRitualCategoryProvider
{
    public BookCategoryModel makeRitualCategory(BookLangHelper helper) {
        helper.category("ritual_category"); //tell our lang helper the category we are in
        ...
        return BookCategoryModel.create(new ResourceLocation(SensMagicMod.MODID, helper.category), helper.categoryName())
                .withIcon(ModBlockss.WIZARD_RITUAL_STONE.getId().toString()); //the icon for the category. In this case we simply use an existing item.
    }
}
